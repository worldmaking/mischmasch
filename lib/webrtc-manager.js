...
class WebrtcManager {
     ...
     */
    constructor(ourPeerId, ourPeerType, signalingChannel, options, verbose, syncStates, docId, emitMessage) {
        ...
        // this.emitMessage.on('msg', e=> console.log('received message', e))
        // this.options.dataChannelHandler.onMessage = (e) =>{

        //     console.log('received msg', e)
        // }
        // emit datachannel connection-related messages to Patch.js
        // this.signalingChannel.onMessage = (e) =>{
        //     e.arg = 'signallingMessage'
        //     e.timestamp = new Date().toLocaleTimeString()
        //     // console.log('received msseg', e)
        //     emitMessage.emit('msg', e)
        // }
        // todo:
        // emit automerge sync protocol messages to patch.js
    }
    /**
    ...
    signalingMessageHandler(message) {
        ...
        switch (action) {
            case "open":
                connections.forEach((newPeer) => {
                    console.log('open')
                    this.addPeer(newPeer.peerId, newPeer.peerType, bePolite, newPeer.canTrickleIceCandidates)
    ...
    addPeer(peerId, peerType, polite, canTrickleIceCandidates = true) {
        if (peerId in this.peers) {
            ...
            if (this.options.enableDataChannel) {
                ...
                try {
                    console.log(this.options.dataChannelHandler)
                    this.options.dataChannelHandler(this.ourPeerId, this.ourPeerType, this.peers[peerId], this.syncStates, this.docId, this.emitMessage);
                } catch (error) {
                    console.error(error);
                }
            }
            ...
            // add peer to automerge syncing
            // create a new sync state for peer
            this.syncStates[peerId] = {}
            this.syncStates[peerId][this.docId] = Automerge.initSyncState()
