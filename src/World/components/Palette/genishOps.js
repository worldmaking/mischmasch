...
const fs = require('fs')
// let ops = Object.keys(opsList)
let newList = [];
for(let i = 0; i< opsList.length; i++){
  let thisOp = opsList[i]
  let cat;
  switch(thisOp.category){
    case 'trigonometry':
    case 'numeric':
    case 'math':
    case 'comparison':
    ...
    default: cat = thisOp.category

  }
  
  let inputs = {}
  Object.keys(thisOp.inputs).forEach(key => {
    inputs[thisOp.inputs[key].name] = thisOp.inputs[key].label
  })

  let outputs = {}
  Object.keys(thisOp.outputs).forEach(key => {
    outputs[thisOp.outputs[key].name] = thisOp.outputs[key].label
  })
...
newList.sort((a, b) => a.classification.localeCompare(b.classification))

console.log(newList)

fs.writeFileSync('ops.json', JSON.stringify(newList, null, 2))
