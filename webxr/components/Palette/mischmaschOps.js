...
let opsList = []
Object.keys(mischMaschOps).forEach(key => {
    let op = {
        op: key,
        classification: mischMaschOps[key].classification,
        inputs: mischMaschOps[key].inputs,
        outputs: mischMaschOps[key].outputs
    }
    opsList.push(op)
})
let fs = require('fs')
fs.writeFileSync('out.json', JSON.stringify(opsList))
console.log(opsList)